...
Here is a summary of the project architecture and technical requirements, optimized for an agentic LLM code editor to begin implementation:

**Project Overview**
The objective is to build an autonomous, agentic real estate discovery platform for a hackathon. The team consists of two developers working in parallel via strict API contracts: Richard handles the backend, algorithms, and data parsing, while Avi handles the frontend, Mapbox view, and chatbot UI.

**Core Tech Stack**
*   **Backend:** Python, FastAPI, OpenAI/Anthropic SDK.
*   **Frontend:** React (Next.js), Mapbox GL JS, Tailwind CSS.
*   **Data Sources:** RentCast API (apartment listings), Overpass API (OpenStreetMap amenities).
*   **Agentic Framework:** Mapbox Model Context Protocol (MCP) Server.

**Pipeline & Architectural Phases**

**1. User Preference Extraction (Backend)**
*   The system must use an LLM with structured outputs to convert natural language user prompts into a strictly defined JSON schema `[1]`. 
*   The schema separates explicit hard constraints (e.g., maximum price, minimum bedrooms) from weighted lifestyle categories (e.g., proximity to schools, parks).
*   Lifestyle categories must be assigned a normalized mathematical weight between 0.0 and 1.0 based on user sentiment and urgency.

**2. Data Aggregation (Backend)**
*   Fetch candidate apartment listings matching the hard constraints using the RentCast API, which provides coordinates, pricing, and property attributes `[2]`. 
*   Assign a unique cryptographic hash (e.g., SHA-256) to each listing based on its coordinates and address to serve as a primary key.
*   Dynamically generate an Overpass QL script based on the LLM-extracted categories to fetch local amenities from OpenStreetMap within a bounding box. Use the `out center;` command to collapse complex OSM polygons into single coordinate points.

**3. Algorithmic Ranking (Backend)**
*   Implement a Weighted Linear Combination (WLC) ranking model `[3]`.
*   Calculate the distance from each apartment to the nearest requested amenities. Apply a negative exponential distance decay function so that extreme proximity is rewarded heavily, but the score decays rapidly as walking distance increases `[3]`. 
*   Apply a linear penalty for higher prices relative to the user's budget.
*   Synthesize the geographic and price scores, and apply Min-Max normalization to output a final algorithmic score strictly between 0.0 and 1.0.

**4. Data Handoff & Schema (Backend to Frontend)**
*   Translate the internal Python ranking dictionary into a standard GeoJSON `FeatureCollection` `[4]`. 
*   Embed the unique `id` at the root of each GeoJSON `Feature`, and inject the normalized score, price, and category breakdown into the `properties` object.

**5. Mapbox Visualization (Frontend)**
*   Store the GeoJSON payload in a React state and synchronize it with the map using the `setData()` method on the Mapbox source `[5]`. 
*   Use Mapbox GL JS expressions to apply data-driven styling, such as interpolating the circle color and radius directly based on the normalized ranking score embedded in the properties `[6]`.
*   Utilize Mapbox's `setFeatureState` API alongside the unique IDs to create high-performance interactive hover effects and popups that display the ranking justifications `[7]`.

**6. Agentic Chatbot Loop**
*   Integrate the Mapbox Model Context Protocol (MCP) Server to give the LLM programmatic access to the location platform `[8]`.
*   Pass the current active Mapbox viewport and the highest-ranked apartment IDs into the LLM's context.
*   Allow the LLM to autonomously trigger MCP tools (like Geocoding, Directions, and Matrix APIs) to answer multi-turn user follow-up questions (e.g., "Which of these is closest to my office?") `[8]`.
*   Actuate the frontend UI (e.g., triggering a camera `flyTo` animation) based on the agent's spatial reasoning `[8]`.
...
Yes, you can consolidate everything into React, but with one critical security constraint: you **must not** make calls to OpenAI or other authenticated APIs directly from client-side browser code, as this will expose your private API keys to the public ``. 

For a severe 1-hour time limit, the absolute best design choice is to use a full-stack React framework like **Next.js** ``. Next.js allows you to build your entire application in one repository, bridging the gap between frontend and backend instantly while maintaining security ``.

Here is the optimal 1-hour execution strategy:

1. **Use Next.js API Routes for Security & CORS:** Instead of building and hosting a separate Python backend, use Next.js Serverless API routes (e.g., `app/api/chat/route.ts`) ``. You can securely store your OpenAI and RentCast API keys in your server environment variables ``. Furthermore, calling external APIs (like OpenStreetMap's Overpass) directly from the browser often results in fatal CORS (Cross-Origin Resource Sharing) errors ``. Routing these data requests through your Next.js API routes cleanly bypasses these browser restrictions ``.
2. **Leverage the Vercel AI SDK:** Do not try to build a custom agentic loop or streaming protocol from scratch. Use the Vercel AI SDK (specifically the `useChat` hook), which is purpose-built for React ``. It automatically handles the streaming conversation state between your frontend and the OpenAI API route, and it has native, out-of-the-box support for tool calling (functions) ``. 
3. **Client-Side Mapbox State:** Keep the Mapbox GL JS map rendering strictly on the client side. When the user interacts with the chat, the Vercel AI SDK triggers a tool call on your Next.js API route. The route fetches the apartment and amenity data, calculates the simple distances, constructs the GeoJSON `FeatureCollection`, and returns it to the client. React then automatically updates the map viewport using the `setData()` method on your existing Mapbox source `[1, 2]`. 

This unified architecture eliminates the need to manage two separate repositories, avoids the overhead of defining strict cross-team API contracts, and prevents you from getting blocked by CORS errors during the demo, making it the only viable path to a functional MVP within 60 minutes ``.